...
from objects import Grumpy, Pipe, Base, Score, Background
    ...
    if start_screen:
        ...
        if game_started and not game_over:
            ...
            if next_pipe - last_pipe >= pipe_frequency:
                ...
                if background.image == background.day_image:
                    pipe_color = 'red'
                else:
                    pipe_color = 'gold'
                
                top = Pipe(win, height, 1, pipe_color)
                bottom = Pipe(win, height, -1, pipe_color)
                pipe_group.add(top)
